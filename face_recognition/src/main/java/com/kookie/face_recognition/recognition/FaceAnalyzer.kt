...
internal class FaceAnalyzer (
    ...
    private val successListener = OnSuccessListener<List<Face>>{faces ->
        ...
        if (face != null) {
            if (detectStatus == FaceAnalyzerStatus.Undetect){
                ...
                listener?.detectProgress(50F, "얼굴을 인식했습니다. \n오른쪽 눈만 깜빡여주세요.")
                ...
                && (face.smilingProbability ?: 0F) > SMILE_SUCCESS_VALE){
                detectStatus =FaceAnalyzerStatus.Smile
                listener?.detectProgress(100F,"얼굴인식이 완료되었습니다.")
                listener?.stopDetect()
                detector.close()
            }
            calDetctSize(face)
    ...
    private fun calDetctSize(face:Face){
    ...
    companion object {
        ...
        private const val SMILE_SUCCESS_VALE = 0.8F
